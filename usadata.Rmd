...
```{r echo=FALSE}
ggplot(usa,aes(x=densityT,y=suicide)) + geom_point(size=3,shape=19,colour="red") + 
geom_smooth(method=lm) + labs(x="Population Density",y="Suicides per 100,000") + 
...
```{r echo=FALSE}
suicideFit <- lm(suicide ~ pc1 + pc2,data=usa)
summary(suicideFit)
```
...
usaF <- principal(usa[,c(3,4,5,6,7,8)], nfactors=2, rotate="promax")
usa$pc1 <- usaF$scores[,1]
usa$pc2 <- usaF$scores[,2]
